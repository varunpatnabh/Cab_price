...
def Confidence_Life_Style_Index(df):
    Confidence_Life_Style_Index_dict = {
                                        'A' : 1,
                                        'B' : 2,
                                        'C' : 3,
                                        'D' : 4
    ...
    return Confidence_Life_Style_Index_dict
...
def convert_nominal_columns(df,target_col):
    '''
        this function will be converting categorical to numerical
        param : Dataframe
        return :dataframe with all numerical values
    '''
    cat_col = ['Type_of_Cab','Destination_Type']
    encoding_cat_dict ={}
    for col in cat_col:
        encoding_cat_dict[col+'_dict'] = encoding(df,col,target_col)

    return encoding_cat_dict


def convert_ordinal(df):
    Confidence_Life_Style_Index_dict = Confidence_Life_Style_Index(df)
    Gender_dict = Gender(df)
    

    # creating combined dictionary
    ordinal_dict= {"Confidence_Life_Style_Index_dict" : Confidence_Life_Style_Index_dict,
